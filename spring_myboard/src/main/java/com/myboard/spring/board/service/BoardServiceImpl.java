...
import com.myboard.spring.board.dao.BoardCommentDao;
import com.myboard.spring.board.dao.BoardDao;
import com.myboard.spring.board.dto.BoardCommentDto;
import com.myboard.spring.board.dto.BoardDto;
import com.myboard.spring.exception.NotDeleteException;
...
public class BoardServiceImpl implements BoardService{
	...
	public void saveContent(BoardDto dto) {
		//title, content, writer 정보가 들어 있는 BoardDto 를 dao 에 전달해서 DB 에 저장되도록한다.
		boardDao.insert(dto);
	}
...
//			throw new NotDeleteException("본인 이외의 유저는 게시물을 마음대로 삭제할 수 없습니다.");
//		}
//		//글 삭제하기 
//		boardDao.delete(num);
//	}
	...
	public void saveComment(HttpServletRequest request) {
		// 폼 전송되는 파라미터 추출
		int ref_group=Integer.parseInt(request.getParameter("ref_group")); // 원글의 글 번호
		String target_id=request.getParameter("target_id"); // 댓글 대상자의 아이디
		String content=request.getParameter("content"); // 댓들의 내용
		/*
	...
	public void deleteComment(HttpServletRequest request) {
		...
		if(!dto.getWriter().equals(id)) {
			throw new NotDeleteException("님의 댓글 지우면 혼나용");
		}
